...
// DeployImgTree will deploy the given image into the given root tree and stores source metadata in info. Returns
...
func (e *Elemental) DeployImage(img *v1.Image, info interface{}) error {
	e.config.Logger.Infof("Deploying image: %s", img.File)
	cleaner, err := e.DeployImgTree(img, cnst.WorkingImgDir, info)
	if err != nil {
		return err
	}
