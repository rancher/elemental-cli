		...
		Describe(fmt.Sprintf("Booting from %s", constants.ActiveLabel), Label("active_label"), func() {
			...
			BeforeEach(func() {
				...
				runner.SideEffect = func(command string, args ...string) ([]byte, error) {
					...
					if command == "mv" && args[0] == "-f" && args[1] == spec.ActiveImg.File && args[2] == activeImg {
						// we doing the image substitution, do the "move"
						source, _ := fs.ReadFile(spec.ActiveImg.File)
						_ = fs.WriteFile(activeImg, source, constants.FilePerm)
						_ = fs.RemoveAll(spec.ActiveImg.File)
					}
					return []byte{}, nil
				}
			...
			It("Successfully upgrades from docker image", Label("docker"), func() {
				spec.ActiveImg.Source = v1.NewDockerSrc("alpine")
				upgrade = action.NewUpgradeAction(config, spec)
				err := upgrade.Run()
				...
				Expect(info.Size()).To(BeNumerically("==", int64(spec.ActiveImg.Size*1024*1024)))
				...
				_, err = fs.Stat(spec.ActiveImg.File)
				Expect(err).To(HaveOccurred())
			})
			It("Successfully upgrades from directory", Label("directory"), func() {
				...
				spec.ActiveImg.Source = v1.NewDirSrc(dirSrc)
		...
		Describe(fmt.Sprintf("Booting from %s", constants.RecoveryLabel), Label("recovery_label"), func() {
			Describe("Using squashfs", Label("squashfs"), func() {
				...
				BeforeEach(func() {
					...
					spec.RecoveryImg.Source = v1.NewChannelSrc("system/cos-config")
					spec.RecoveryImg.Size = 16
					...
					runner.SideEffect = func(command string, args ...string) ([]byte, error) {
						...
						if command == "mv" && args[0] == "-f" && args[1] == spec.RecoveryImg.File && args[2] == recoveryImgSquash {
							// fake "move"
							f, _ := fs.ReadFile(spec.RecoveryImg.File)
							_ = fs.WriteFile(recoveryImgSquash, f, constants.FilePerm)
							_ = fs.RemoveAll(spec.RecoveryImg.File)
						}
						return []byte{}, nil
					}
					config.Runner = runner
				...
				It("Successfully upgrades recovery from docker image", Label("docker"), func() {
					...
					spec.RecoveryImg.Source = v1.NewDockerSrc("alpine")
					upgrade = action.NewUpgradeAction(config, spec)
					err = upgrade.Run()
					Expect(err).ToNot(HaveOccurred())
					...
					info, err = fs.Stat(spec.RecoveryImg.File)
					Expect(err).To(HaveOccurred())
				...
				It("Successfully upgrades recovery from directory", Label("directory"), func() {
					...
					spec.RecoveryImg.Source = v1.NewDirSrc(srcDir)
					upgrade = action.NewUpgradeAction(config, spec)
					err := upgrade.Run()
					Expect(err).ToNot(HaveOccurred())
			...
			Describe("Not using squashfs", Label("non-squashfs"), func() {
				...
				BeforeEach(func() {
					...
					runner.SideEffect = func(command string, args ...string) ([]byte, error) {
						...
						if command == "mv" && args[0] == "-f" && args[1] == spec.RecoveryImg.File && args[2] == recoveryImg {
							// fake "move"
							f, _ := fs.ReadFile(spec.RecoveryImg.File)
				...
				It("Successfully upgrades recovery from docker image", Label("docker"), func() {
					...
					// Image size should not be empty
					Expect(info.Size()).To(BeNumerically(">", 0))
					Expect(info.Size()).To(BeNumerically("<", int64(spec.RecoveryImg.Size*1024*1024)))
					Expect(info.IsDir()).To(BeFalse())
					f, _ := fs.ReadFile(recoveryImg)
					Expect(f).To(ContainSubstring("recovery"))

					spec.RecoveryImg.Source = v1.NewDockerSrc("apline")
