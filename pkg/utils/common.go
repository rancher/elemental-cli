...
// MarshalToYaml marshals the given object into the given YAML file
func MarshalToYaml(fs v1.FS, object interface{}, file string, editor ...func([]byte) []byte) error {
...
// UnmarshalFromYaml loads the given yaml file into the given object
func UnmarshalFromYaml(fs v1.FS, object interface{}, file string) error {
...
// WritePartitionState writes the partition state meta-file to the given partition mountpoint
func WritePartitionState(fs v1.FS, mountPoint string, partState *v1.PartitionState) error {
	editor := func(data []byte) []byte {
		data = append([]byte("# Autogenerated file by elemental client, do not edit\n\n"), data...)
		return data
	}
	return MarshalToYaml(fs, partState, filepath.Join(mountPoint, constants.PartitionMetaFile), editor)
}

// LoadPartitionState gets the partition state from the given partition mountpoint by loading
// the partition state meta-file
func LoadPartitionState(fs v1.FS, mountPoint string) (*v1.PartitionState, error) {
	partState := &v1.PartitionState{}
	err := UnmarshalFromYaml(fs, partState, filepath.Join(mountPoint, constants.PartitionMetaFile))
	if err != nil {
		return nil, err
	}
